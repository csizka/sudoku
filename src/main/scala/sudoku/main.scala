...
def main(): Unit = {
  ...
  assert(finishSudoku(oneCellMissingSudoku) == Option(goodSudoku))
  assert(finishSudoku(someCellMissingSudoku) == Option(goodSudoku))
  assert(finishSudoku(partiallySolvedGoodSudoku) == Option(goodSudoku))
