...
object PlaySudoku {
  ...
  def parseInsertCmd(args: List[Char]): Either[String, Command] = args.map(x => x.toInt - 49) match {
    ...
    case _ => Left(
      s"Insertion could not be completed with the command i$args. (>_<) " +
  ...
  def parseDelCmd(args: List[Char]): Either[String, Command] = args.map(x => x.toInt - 49) match {
    ...
    case _ => Left(
      s"Deletion could not be completed with command: d$args. (u_u)" +
  ...
  def execCommand(curSudoku: Sudoku, cmd: Command, startSudoku: Sudoku): Either[String, Sudoku] = cmd match {
    ...
    case Delete(rowIx, colIx) =>
      if (cellIsEmpty(startSudoku, rowIx, colIx)) {
        ...
        Left(s"Deletion could not be completed with command: d${rowIx + 1}${colIx + 1}. (u_u) The 2 numbers after the letter 'd' need to be between 1 and 9, and they can only point to a cell, that was empty at the beginning of the game. Please try something else.")
      }
    case Restart() =>
      ...
      response match {
        ...
        case "n" =>
          ...
          Right(curSudoku)
        case _   =>
          Left(s"Your command:$response was not understood, therefore you are resuming to the previous state the game.")
      }
  ...
  def choosingNextMove(curSudoku: Sudoku, name: String, startSudoku: Sudoku): Sudoku = {
    if (isSudokuSolved(curSudoku)) {
      ...
      println("1st char => action: i = insert, d = delete, r = restart")
